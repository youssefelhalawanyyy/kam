...
import { Mail, Lock, Eye, EyeOff, User, Store, Loader2, ArrowRight, Check } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (subscribe) await addSubscriber(email, name);
      toast.success("Account created! Welcome to BKAM.");
      router.push("/");
    } catch (err: any) {
      const msg =
  ...
  const handleApple = async () => {
    ...
    try {
      ...
      if (err.code !== "auth/popup-closed-by-user") toast.error("Apple sign-in failed");
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-20"
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 w-full max-w-md">
        ...
        <div className="rounded-3xl p-8 shadow-2xl"
          ...
          <p className="text-sm mb-6" style={{ color: "#9ca3af" }}>Join Egypt's smartest price platform</p>

          {/* Account Type Toggle */}
          <div className="flex gap-2 p-1 rounded-xl mb-6" style={{ background: "rgba(255,255,255,0.07)" }}>
            {(["user", "store"] as AccountType[]).map((type) => (
              <button key={type} type="button" onClick={() => setAccountType(type)}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-semibold transition-all"
                style={accountType === type
                  ? { background: "#22C55E", color: "white" }
                  : { color: "#9ca3af" }}>
                {type === "user" ? <User className="w-4 h-4" /> : <Store className="w-4 h-4" />}
                {type === "user" ? "Shopper" : "Store Owner"}
              </button>
            ))}
          </div>
          ...
          <div className="space-y-3 mb-6">
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
              onClick={handleGoogle} disabled={googleLoading} type="button"
            ...
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
              onClick={handleApple} disabled={appleLoading} type="button"
              className="w-full flex items-center justify-center gap-3 py-3 rounded-xl font-semibold text-sm text-white transition-all disabled:opacity-60"
              style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)" }}>
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <span className="text-xs" style={{ color: "#6b7280" }}>or with email</span>
            <div className="flex-1 h-px" style={{ background: "rgba(255,255,255,0.1)" }} />
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            ...
            <div className="relative">
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} required
                ...
                className="w-full pl-11 pr-4 py-3 rounded-xl text-white placeholder-gray-500 text-sm focus:outline-none transition-all"
            ...
            <div>
              <div className="relative">
                ...
                <input type={showPass ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} required
                  ...
                  className="w-full pl-11 pr-12 py-3 rounded-xl text-white placeholder-gray-500 text-sm focus:outline-none transition-all"
                  style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)" }} />
                ...
                <div className="flex gap-1 mt-2">
                  {[1, 2, 3].map((l) => (
                    <div key={l} className="h-1 flex-1 rounded-full transition-all"
                      style={{ background: passwordStrength >= l ? (l === 1 ? "#ef4444" : l === 2 ? "#f97316" : "#22C55E") : "rgba(255,255,255,0.1)" }} />
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="relative">
              ...
              {confirmPass && (
                <div className="absolute right-4 top-1/2 -translate-y-1/2"
                  style={{ color: confirmPass === password ? "#22C55E" : "#ef4444" }}>
            ...
            <label className="flex items-start gap-3 cursor-pointer">
              <div onClick={() => setSubscribe(!subscribe)}
                ...
                style={subscribe ? { background: "#22C55E", border: "2px solid #22C55E" } : { border: "2px solid rgba(255,255,255,0.3)", background: "transparent" }}>
                {subscribe && <Check className="w-3 h-3 text-white" />}
              </div>
              <span className="text-sm" style={{ color: "#9ca3af" }}>Get the best deals & price alerts via email</span>
            </label>

            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
              type="submit" disabled={loading}
              className="w-full py-3.5 font-bold rounded-xl text-white flex items-center justify-center gap-2 text-sm disabled:opacity-60"
          ...
          <p className="text-center text-sm mt-5" style={{ color: "#9ca3af" }}>
            ...
            <Link href="/auth/login" className="font-semibold hover:opacity-80" style={{ color: "#22C55E" }}>Sign in</Link>
          </p>
        </div>
      </motion.div>
    </div>
